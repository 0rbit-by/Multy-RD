...
public class Player {
   ...
   private void resetPos() {
      ...
      int y = this.level.depth - 1;

      while (y > 0 && !this.level.isTile(x, y, z)) {
         y--;
      }

      this.setPos((float)x, (float)(y + 3), (float)z);
   }
   ...
   public void tick() {
      ...
      if (this.flying) {
         this.yd = 0.0F;
         if (Keyboard.isKeyDown(57)) this.yd = 0.1F;
         if (Keyboard.isKeyDown(42)) this.yd = -0.1F;
      } else {
      ...
      float speed = this.flying ? 0.03F : (this.onGround ? 0.02F : 0.008F);
